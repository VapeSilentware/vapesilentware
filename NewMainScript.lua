...
task.spawn(function() pcall(function() if isfile("SW_API_KEY.txt") then delfile("SW_API_KEY.txt") end end) end)
    ...
    if not suc or res == "404: Not Found" then
        if isImportant then
            game:GetService('StarterGui'):SetCore('SendNotification', {
				Title = 'Failure loading Silentware | Please try again',
...
local function pload(fileName, isImportant, required)
    ...
    if string.find(fileName, "CustomModules") and string.find(fileName, "Silentware") then
        fileName = string.gsub(fileName, "Silentware", "SW")
    end        
