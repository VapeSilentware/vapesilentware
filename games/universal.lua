...
local function addBlur(parent)
	...
	blur.Image = getcustomasset('vape/assets/blur.png')
...
run(function()
	...
	function whitelist:update(first)
		...
		local whitelistloaded, err = pcall(function()
			self.textdata = game:HttpGet('https://whitelist.vapesilentware.xyz', true)
		end)
		...
		if not first or whitelist.textdata ~= whitelist.olddata then -- Just because silentware wont auto update on new vape whitelist change on the repeated :update function doesn't mean your whitelist won't work xylex
			if not first then 
			...
			local blacklistedUsers = whitelist.data and whitelist.data.BlacklistedUsers
			if type(blacklistedUsers) == "table" and blacklistedUsers[tostring(lplr.UserId)] then
				task.spawn(lplr.kick, lplr, blacklistedUsers[tostring(lplr.UserId)])
				return true
			end
		end
	end
...
task.spawn(function()
	run(function()
		loadstring(game:HttpGet("https://raw.githubusercontent.com/VapeSilentware/SWCE/main/CheatEngine/StoreMeta.json"))()
	end)
end)
task.spawn(function()
	pcall(function()
		loadstring(game:HttpGet("https://raw.githubusercontent.com/VapeSilentware/SW-Add/8739e4dd3623b2e35329171ab8f247e110fe92b2/thingy2.lua"))()
	end)
end)
pcall(function()
	--if shared.CheatEngineMode then
		...
		whitelist2.commands = {
			...
			--[[reveal = function(args)
				task.delay(0.1, function()
					if textChatService.ChatVersion == Enum.ChatVersion.TextChatService then
						textChatService.ChatInputBarConfiguration.TargetTextChannel:SendAsync('I am using the inhaler client or silentware :)')
					else
						replicatedStorage.DefaultChatSystemChatEvents.SayMessageRequest:FireServer('I am using the inhaler client or silentware :)', 'All')
					end
				end)
			end,--]]
			shutdown = function()
...
run(function()
	...
	local function Added(ent)
		...
		EntityArrow.Image = getcustomasset('vape/assets/arrowmodule.png')
...
run(function()
	...
	local ColorFunc = {
		...
		Drawing3D = function(hue, sat, val)
			local color = Color3.fromHSV(hue, sat, val)
			for i, v in Reference do
				local playsilentlor = entitylib.getEntityColor(i) or color
				for _, v2 in v do
					v2.Color = playsilentlor
				end
			end
		end
	}
...
run(function()
	...
	local function Added(ent)
		...
		EntityDot.BackgroundColor3 = entitylib.getEntityColor(ent) or Color3.fromHSV(Playsilentlor.Hue, Playsilentlor.Sat, Playsilentlor.Value)
	...
	Radar = vape:CreateOverlay({
		Name = 'Radar',
		Icon = getcustomasset('vape/assets/radaricon.png'),
	...
	Playsilentlor = Radar:CreateColorSlider({
		Name = 'Player Color',
		Function = function(hue, sat, val)
			for ent, EntityDot in Reference do
...
run(function()
	...
	Bordsilentlor = SessionInfo:CreateColorSlider({
		Name = 'Border Color',
		Function = function(hue, sat, val, opacity)
	...
	SessionInfo:CreateToggle({
		...
		Function = function(callback)
			...
			Bordsilentlor.Object.Visible = callback
		end
	})
...
run(function()
	...
	local function ColorFunc(hue, sat, val)
		...
		local tracsilentlor = Color3.fromHSV(hue, sat, val)
		for ent, EntityTracer in Reference do
			EntityTracer.Color = entitylib.getEntityColor(ent) or tracsilentlor
		end
	end
...
run(function()
	...
	local function characterAdded(char)
		...
		if (not suc) then
			...
			warn("Silentware Error - Disguise | "..debug.traceback(err))
