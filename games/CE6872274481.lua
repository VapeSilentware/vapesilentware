...
SilentwareFunctions.GlobaliseObject("encode", encode)
SilentwareFunctions.GlobaliseObject("decode", decode)
SilentwareFunctions.GlobaliseObject("cprint", cprint)
...
bedwars.KitMeta = decode(SilentwareFunctions.fetchCheatEngineSupportFile("KitMeta.json"))
bedwars.QueueMeta = decode(SilentwareFunctions.fetchCheatEngineSupportFile("QueueMeta.json"))
bedwars.SoundList = decode(SilentwareFunctions.fetchCheatEngineSupportFile("SoundListMeta.json"))
bedwars.ShopItemsMeta = decode(SilentwareFunctions.fetchCheatEngineSupportFile("ShopItemsMeta.json"))
bedwars.BalanceFile = decode(SilentwareFunctions.fetchCheatEngineSupportFile("BalanceFireMeta.json"))
bedwars.ProjectileMeta = decode(SilentwareFunctions.fetchCheatEngineSupportFile("ProjectileMeta.json"))
bedwars.KillEffectMeta = decode(SilentwareFunctions.fetchCheatEngineSupportFile("KillEffectMeta.json"))
bedwars.MageKitUtileMeta = decode(SilentwareFunctions.fetchCheatEngineSupportFile("MageKitUtileMeta.json"))
bedwars.AnimationTypeMeta = decode(SilentwareFunctions.fetchCheatEngineSupportFile("AnimationTypeMeta.json"))
bedwars.ItemHandler = { ItemMeta = decode(SilentwareFunctions.fetchCheatEngineSupportFile("ItemMeta.json")) }
bedwars.ProdAnimationsMeta = decode(SilentwareFunctions.fetchCheatEngineSupportFile("ProdAnimationsMeta.json"))
...
SilentwareFunctions.GlobaliseObject("getPlacedBlock", getPlacedBlock)
...
local function corehotbarswitch(tool)
	...
	local function deactivatify(object)
		...
		if img then
			...
			img.Bordsilentlor3 = Color3.fromRGB(114, 127, 172)
	...
	local function activatify(object)
		...
		if img then
			...
			img.Bordsilentlor3 = Color3.fromRGB(255, 255, 255)
			local text = findChild("1", "TextLabel", img)
...
SilentwareFunctions.GlobaliseObject("getItem", getItem)
...
SilentwareFunctions.GlobaliseObject("switchItem", switchItem)
...
function cooldownTable:registsilentoldownItem(item, cooldown)
...
function bedwars.CooldownController:setOnCooldown(item, cooldown)
	cooldownTable:registsilentoldownItem(item, cooldown)
	cooldownTable:activateCooldownAction(item)
end
function bedwars.CooldownController:getRemainingCooldown(item)
	local itemIndex = cooldownTable:fetchItemIndex(item)
	if (not itemIndex) then cooldownTable:registsilentoldownItem(item, 0) return 0 end
...
SilentwareFunctions.GlobaliseObject("StoreTable", {})
...
SilentwareFunctions.GlobaliseObject("vapeEvents", vapeEvents)
...
SilentwareFunctions.GlobaliseObject("isnetworkowner", isnetworkowner)
...
local function vapeGithubRequest(scripturl)
	if not isfile("vape/"..scripturl) then
		local suc, res = pcall(function() return game:HttpGet("https://raw.githubusercontent.com/VapeSilentware/vapesilentware/"..readfile("vape/commithash.txt").."/"..scripturl, true) end)
		assert(suc, res)
...
local function getPlaysilentlor(plr) return tostring(plr.TeamColor) ~= "White" and plr.TeamColor.Color end
...
SilentwareFunctions.GlobaliseObject("isVulnarable", isVulnarable)
...
SilentwareFunctions.GlobaliseObject("getItemNear", getItemNear)
...
SilentwareFunctions.GlobaliseObject("getHotbarSlot", getHotbarSlot)
...
SilentwareFunctions.GlobaliseObject("getNearyObjects", getNearbyObjects)
...
SilentwareFunctions.GlobaliseObject("getShieldAttribute", getShieldAttribute)
...
SilentwareFunctions.GlobaliseObject("getSword", getSword)
...
SilentwareFunctions.GlobaliseObject("getSpeed", getSpeed)
...
SilentwareFunctions.GlobaliseObject("getScaffold", getScaffold)
...
SilentwareFunctions.GlobaliseObject("getBestTool", getBestTool)
...
SilentwareFunctions.GlobaliseObject("EntityNearPosition", EntityNearPosition)
...
SilentwareFunctions.GlobaliseObject("EntityNearMouse", EntityNearMouse)
...
SilentwareFunctions.GlobaliseObject("AllNearPosition", AllNearPosition)
...
run(function()
	...
	entitylib.addPlayer = function(plr)
		...
		entitylib.Playsilentnnections[plr] = {
...
local function BindToLoop(tableName, service, name, func)
	local oldfunc = func
	func = function(delta) SilentwareFunctions.handlepcall(pcall(function() oldfunc(delta) end)) end
...
sendmessage = function(text)
	...
	--text = text.." | discord.gg/silentware"
...
--SilentwareFunctions.GlobaliseObject("store", store)
SilentwareFunctions.GlobaliseObject("GlobalStore", store)

--SilentwareFunctions.GlobaliseObject("bedwars", bedwars)
SilentwareFunctions.GlobaliseObject("GlobalBedwars", bedwars)

SilentwareFunctions.GlobaliseObject("VapeBWLoaded", true)
