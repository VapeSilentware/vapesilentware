...
local function run(func)
	...
	if err then warn("[SW6872265039.lua Module Error]: "..tostring(debug.traceback(err))) end
end
...
SilentwareFunctions.GlobaliseObject("BedwarsInfoNotification", BedwarsInfoNotification)
...
SilentwareFunctions.GlobaliseObject("BedwarsErrorNotification", BedwarsErrorNotification)
...
if shared.TeleportExploitAutowinEnabled then
	...
	--[[local function InfoNotification2(title, text, delay, button_table)
		local suc, res = pcall(function()
			local frame = GuiLibrary.CreateInteractableNotification(title or "Silentware", text or "Successfully called function", delay or 7, "assets/InfoNotification.png", button_table)
			return frame
		end)
		return (suc and res)
	end
...
--[[run(function()
    ...
    local RankIconTable = {["Emerald"] = "rbxassetid://12599231807",["Nightmare"] = "rbxassetid://7904292926",["Silentware"] = "rbxassetid://18518244636"}
    ...
    local function registerType(typeName)
        ...
        elseif typeName == "RankIcon" then LeaderboardEditor_Editors["RankIcon"] = {Value = "Silentware"}; Leaderboard_AddOnCreator_Helper["RankIcon"] = {Type = "CreateDropdown", Args = {Name = "RankIcon", List = fetchRankIconNames(), Function = fetchDefaultFunction}, ResType = "Value"};
...
--[[run(function()
    ...
    local function updateUsers(users)
        ...
        for index, user in ipairs(users) do
            ...
            statusIndicator.Bordsilentlor3 = Color3.fromRGB(255, 255, 255)
