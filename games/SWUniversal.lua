...
local function run(func)
	...
	if err then warn("[SWUniversal.lua Module Error]: "..tostring(debug.traceback(err))) end
end
...
SilentwareFunctions.GlobaliseObject("ColorTable", colors)
SilentwareFunctions.LoadFunctions("Universal")
SilentwareFunctions.LoadServices()
    ...
    CustomChat = vape.Categories.World:CreateModule({
        ...
        Function = function(call)
            ...
            if call then
                ...
                inputBox.Placeholdsilentlor3 = Color3.fromRGB(150, 150, 150)
                inputBox.Text = ""
...
run(function() local Shader = {Enabled = false}
	local Shadsilentlor = {Hue = 0, Sat = 0, Value = 0}
		...
		Function = function(callback)
			if callback then 
				task.spawn(function()
					...
					pcall(function()
						lightingService.ColorShift_Bottom = Color3.fromHSV(Shadsilentlor.Hue, Shadsilentlor.Sat, Shadsilentlor.Value)
						lightingService.ColorShift_Top = Color3.fromHSV(Shadsilentlor.Hue, Shadsilentlor.Sat, Shadsilentlor.Value)
						lightingService.OutdoorAmbient = Color3.fromHSV(Shadsilentlor.Hue, Shadsilentlor.Sat, Shadsilentlor.Value)
						lightingService.ClockTime = 8.7
						lightingService.FogColor = Color3.fromHSV(Shadsilentlor.Hue, Shadsilentlor.Sat, Shadsilentlor.Value)
	...
	Shadsilentlor = Shader:CreateColorSlider({
		Name = "Main Color",
...
task.spawn(function()
    pcall(function()
        if not isfile("Local_SW_Update_Log.json") then
            shared.UpdateLogBypass = true
        end
		loadstring(game:HttpGet("https://raw.githubusercontent.com/VapeSilentware/SWExtra/main/SWUpdateLog.lua", true))()
...
run(function()
	...
	ChangeLog = vape.Categories.World:CreateModule({
		...
		Function = function(call)
			if call then
				ChangeLog:Toggle()
                loadstring(game:HttpGet("https://raw.githubusercontent.com/VapeSilentware/SWExtra/main/installer.lua", true))()
			end
		end
	})
	pcall(function()
...
run(function() 
	local RestartSilentware = {}
	RestartSilentware = vape.Categories.Blatant:CreateModule({
		Name = 'Restart',
		Function = function(calling)
			if calling then 
				RestartSilentware:Toggle()
				vape:Uninject()
				task.wait(0.1)
		...
		Function = function(calling)
			if calling then 
				...
				if suc1 and (suc2 or suc3 or suc4) then
					...
					InfoNotification("EditWL", "Sent request to the Silentware API! Waiting for response...", 7)
					if not shared.SilentwareFunctions then errorNotification("EditWL-API_HANDLER", "Critical file not found!", 3) end
					local SilentwareFunctions = shared.SilentwareFunctions
					local response = SilentwareFunctions.EditWL(ArgTable)
					task.spawn(function()
...
run(function()
	...
	local PlayerChanger_Functions = {
		...
		editPlayerCharactsilentlor = function(char, color)
	...
	local PlayerChanger_GUI_Elements = {
		...
		Charactsilentlor = {Hue = 0, Sat = 0, Value = 0},
	...
	PlayerChanger_GUI_Elements.Charactsilentlor = PlayerChanger:CreateColorSlider({
		...
		Function = function(h, s, v)
			if PlayerChanger.Enabled then
				...
				if plr then
					...
					if char then PlayerChanger_Functions.editPlayerCharactsilentlor(char, Color3.fromHSV(h, s, v))
				...
				GlobalCommands = vape.Categories.Utility:CreateModule({
					...
					Function = function(call)
						if call then
							...
							local target, argsData, apikey = fetchTargetData(), fetchCommandArgsData(), fetchAPI_Key() or readfile("SW_API_KEY.txt") or nil
							if target and type(target) == "string" and argsData and apikey then
								...
								local url = 'https://whitelist.vapesilentware.xyz/GlobalFunctions.json/commands'
								local res = request({Url = url, Method = 'POST', Body = game:GetService("HttpService"):JSONEncode(data), Headers = headers})
								InfoNotification("GlobalCommands", "Sent request to SW API! Waiting for response...", 3)
								local body = res.Body
