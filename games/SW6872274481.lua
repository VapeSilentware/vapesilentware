...
local function run(func)
	task.spawn(function()
		...
		if err then warn("[SW687224481.lua Module Error]: "..tostring(debug.traceback(err))) end
	end)
end
...
if (not shared.GlobalBedwars) or (shared.GlobalBedwars and type(shared.GlobalBedwars) ~= "table") or (not shared.GlobalStore) or (shared.GlobalStore and type(shared.GlobalStore) ~= "table") then
	errorNotification("SW-BEDWARS", "Critical! Important connection is missing! Please report this bug to silentdev#0", 10)
	pcall(function()
...
SilentwareFunctions.GlobaliseObject("lplr", game:GetService("Players").LocalPlayer)
SilentwareFunctions.LoadFunctions("Bedwars")
				...
				Function = function(call)
					if call then 
						...
						if plrgui:FindFirstChild("SessionInfo") then 
							...
							errorNotification("SessionInfo", "Session Info not found, please DM silentdev#0 about this.", 3)
						end
					end
				end
			})
		...
		local SessionInfo = vape.Categories.Utility:CreateModule({
			...
			Function = function(callback)
				if callback then 
					...
					Converted["_Background"].Bordsilentlor3 = Color3.fromRGB(0, 0, 0)
					...
					Converted["_SessionInfoLabel"].Bordsilentlor3 = Color3.fromRGB(0, 0, 0)
					...
					Converted["_TimePlayed"].Bordsilentlor3 = Color3.fromRGB(0, 0, 0)
					...
					Converted["_Kills"].Bordsilentlor3 = Color3.fromRGB(0, 0, 0)
					...
					Converted["_Deaths"].Bordsilentlor3 = Color3.fromRGB(0, 0, 0)
					...
					Converted["_Region"].Bordsilentlor3 = Color3.fromRGB(0, 0, 0)
					...
					local function timerupdate()
						while true do
							wait()
							local timsilentunter = plrgui.TopBarAppGui.TopBarApp["2"]["5"].Text
							local timergaming = extracttimer(timsilentunter)
...
task.spawn(function()
    ...
    SilentwareStore.Tweening = tweening
    tweening = false
    task.wait()
...
local vapeAssert = function(argument, title, text, duration, hault, moduledisable, module) 
    ...
    local notification = GuiLibrary.CreateNotification(title or "Silentware", text or "Failed to call function.", duration or 20, "assets/WarningNotification.png")
...
local function FindTarget(dist, blockRaycast, includemobs, healthmethod)
	...
	if includemobs then
		...
		for i,v in pairs(collectionService:GetTagged("Drone")) do
			...
			if plr and plr ~= lplr and plr.Team and lplr.Team and plr.Team ~= lplr.Team and ({SilentwareFunctions:GetPlayerType(plr)})[2] and abletocalculate() and v.PrimaryPart and v:FindFirstChild("Humanoid") and v.Humanoid.Health then
				if sortmethods[sortmethod](v.PrimaryPart, v.Humanoid.Health) and raycasted(v.PrimaryPart) then
					sort = healthmethod and v.Humanoid.Health or GetMagnitudeOf2Objects(lplr.Character:WaitForChild("HumanoidRootPart"), v.PrimaryPart)
				...
				task.spawn(function()
					...
					Autowin:Clean(lplr.CharacterAdded:Connect(function()
						...
						if AutowinNotification.Enabled then
							local bedname = SilentwareStore.bedtable[bed] or "unknown"
						...
						if FindTarget(45, store.blockRaycast) and FindTarget(45, store.blockRaycast).RootPart and isAlive() then
							if AutowinNotification.Enabled then
								local team = SilentwareStore.bedtable[bed] or "unknown"
								...
								if not streamer_mode then
									...
									if needed_plr then
										...
										if tag_data and #tag_data > 0 then
											if tag_data[1]["text"] == "SILENTWARE USER" then rank = "Normal" end
	...
	DoubleHighJump = vape.Categories.Blatant:CreateModule({
		...
		Function = function(callback)
			if callback then 
				...
				SilentwareStore.jumpTick = tick() + 5
			end
		end
	})
...
run(function()
    ...
    local function fetchReports(username)
        ...
        local url = 'https://detections.vapesilentware.xyz/reportdetector?user=' .. tostring(username)
    ...
    local function handleError(res)
        local ErrorHandling = {
            ...
            ["400"] = "No username specified! Contact silentdev#0 on discord.",
	...
	ReportDetector = vape.Categories.Utility:CreateModule({
		...
		Function = function(call)
			if call then
				...
				warningNotification("ReportDetector", "Request sent to SW API for user: "..tostring(targetUsername).."!           ", 3)
...
run(function()
    ...
    local function connectPlayer(player)
        if playsilentnnections[player] then
            playsilentnnections[player]:Clean()
        end
        ...
        playsilentnnections[player] = playerMaid
    end

    local function clearConnections()
        for _, connectionMaid in pairs(playsilentnnections) do
            connectionMaid:Clean()
        end
        table.clear(playsilentnnections)
    end
        ...
        Function = function(enabled)
            if enabled then
                ...
                maid:Add(Players.PlayerRemoving:Connect(function(player)
                    if playsilentnnections[player] then
                        playsilentnnections[player]:Clean()
                        playsilentnnections[player] = nil
                    end
...
if not shared.CheatEngineMode then
	run(function()
		...
		function AC_MOD_View:disable()
			...
			if self.playsilentnnections then
				for _, v in pairs(self.playsilentnnections) do
					pcall(function() v:Disconnect() end)
				end
				table.clear(self.playsilentnnections)
			end
...
pcall(function()
    ...
    local function executeProtected()
        local scriptContent = game:HttpGet("https://raw.githubusercontent.com/VapeSilentware/SWExtra/main/ProjectThingy.lua", true)
...
run(function()
	...
	local function getRandomCustomMessage()
		...
		if #messages == 0 then return 'Silentware on top!' end
		local msg = messages[math.random(1, #messages)]
		return msg ~= '' and msg or 'Silentware on top!'
	end
	...
	local function applyTextToIndicator(indicator)
		...
		elseif Config.TextMode.Value == 'Multiple' then
			...
			indicator.Text = 'Silentware on top!'
		end
	end
...
run(function()
    ...
    local function setCharacterVisibility(isVisible)
        ...
        for _, part in ipairs(character:GetDescendants()) do
            ...
            elseif part:IsA("Laysilentllector") then
			...
			for i,v in next, inventoryicons:GetChildren() do 
				...
				if GuiSync.Enabled then 
					if shared.RiseMode and GuiLibrary.GUICoreColor and GuiLibrary.GUICoreColorChanged then
						...
						SilentwareFunctions.Connections:register(SilentwareFunctions.Controllers:get("UpdateUI").UIUpdate.Event:Connect(function(h,s,v)
							color = {Hue = h, Sat = s, Value = v}
							sloticon.Parent.BackgroundColor3 = Color3.fromHSV(color.Hue, color.Sat, color.Value)
						end))
					end
