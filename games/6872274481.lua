...
internalConstructor = {
		...
		if key == "new" then
			return function(id)
				if internalConfig.initialisationDebug then
					warn("[SILENTWARE INTERNAL]: New Instance ["..tostring(id).."]")
				end
	...
	__call = function(self, debug)
		if not self.__id then
			warn(debug.traceback("[SILENTWARE INTERNAL]: Called without initialisation!"))
			return
		end
...
local function saveErrorLog(err, S_Name)
	...
	writefile('SW_Error_Log.json', HttpService:JSONEncode(main))
	errorNotification("Silentware -  Error Logger", 'If you can please send the\n SW_Error_Log.json file in your workspace to silentdev#0 or discord.gg/silentware', 10)
...
run(function()
    ...
    local moduleDefinitions = {
        ...
        HudAliveCount = function() return require(lplr.PlayerScripts.TS.controllers.global['top-bar'].ui.game['hud-alive-player-counts']).HudAlivePlaysilentunts end,
    ...
    local remoteDefinitions = setmetatable({
		...
		__tostring = function()
			return "SILENTWARE_INTERNAL_REMOTE_DEFINITIONS"
		end
	})
...
if not bedwars.Client then
	errorNotification('Silentware Bedwars', "There was a critical loading error! \n Please report this issue to silentdev#0 or discord.gg/silentware", 10)
end
assert(bedwars.Client ~= nil and type(bedwars.Client) == "table", "There was a critical loading error! \n Please report this issue to silentdev#0 or discord.gg/silentware")
...
run(function()
	...
	vape.Components.HotbarList = function(optionsettings, children, api)
		...
		textbuttonicon.Image = getcustomasset('vape/assets/add.png')
			...
			local mounted = bedwars.Roact.mount(create('BillboardGui', {
				...
				create('Frame', {
					...
					create('ImageLabel', {
						...
						BackgroundTransparency = 1,
						Image = getcustomasset('vape/assets/blur.png'),
