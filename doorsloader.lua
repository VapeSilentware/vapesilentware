...
if isNew then
    ...
    local suc, data = pcall(function()
        return game:HttpGet("https://raw.githubusercontent.com/Silentbg/SilentwareProfiles/main/mspaint/doors/settings/pro.json", true)
    end)
    if suc then
...
local function load()
    local suc2, err2 = pcall(function() loadstring(game:HttpGet("https://raw.githubusercontent.com/VapeSilentware/SW-Add/main/mspaint.lua", true))() end)
    return {Data1 = suc2, Data2 = err2}
end
local data = load()
if (not data.Data1) then
    errorNotification("Silentware x mspaint - Doors", "Failure loading mspaint! Error: "..tostring(data.Data2), 7)
end