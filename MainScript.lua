...
local function vapeGithubRequest(scripturl)
	if not isfile("vape/"..scripturl) then
		...
		suc, res = pcall(function() return game:HttpGet("https://raw.githubusercontent.com/VapeSilentware/SilentwareBakup/main/"..scripturl, true) end)
...
SWFunctions = pload("Libraries/SilentwareFunctions.lua", true, true)

GuiLibrary.SelfDestructEvent.Event:Connect(function() SWFunctions.SelfDestructEvent:Fire() end)

SWFunctions.GlobaliseObject("GuiLibrary", GuiLibrary)
...
SWFunctions.GlobaliseObject("encode", encode)
local function decode(tbl)
    return game:GetService("HttpService"):JSONDecode(tbl)
end
SWFunctions.GlobaliseObject("decode", decode)
...
SWFunctions.GlobaliseObject("GUI", GUI)
...
ProfilesTextList = Profiles.CreateTextList({
	...
	CustomFunction = function(profileObject, profileName)
		...
		if profileName == GuiLibrary.CurrentProfile then
			profileObject.BackgroundColor3 = Color3.fromHSV(GuiLibrary.ObjectsThatCanBeSaved["Gui ColorSlidsilentlor"].Api.Hue, GuiLibrary.ObjectsThatCanBeSaved["Gui ColorSlidsilentlor"].Api.Sat, GuiLibrary.ObjectsThatCanBeSaved["Gui ColorSlidsilentlor"].Api.Value)
			profileObject.ImageButton.BackgroundColor3 = Color3.fromHSV(GuiLibrary.ObjectsThatCanBeSaved["Gui ColorSlidsilentlor"].Api.Hue, GuiLibrary.ObjectsThatCanBeSaved["Gui ColorSlidsilentlor"].Api.Sat, GuiLibrary.ObjectsThatCanBeSaved["Gui ColorSlidsilentlor"].Api.Value)
...
OnlineProfilesButton.MouseButton1Click:Connect(function()
	...
	if not profilesLoaded then
        ...
        local success, result = pcall(function()
            return game:GetService("HttpService"):JSONDecode(game:HttpGet("https://raw.githubusercontent.com/Silentbg/VapeProfiles/main/Profiles/"..saveplaceid.."/profilelist.txt", true))
        end)
		for i,v in pairs(success and result or {}) do
			onlineprofiles[i] = v
		end
		for i2,v2 in pairs(onlineprofiles) do
			local profileurl = "https://raw.githubusercontent.com/Silentbg/VapeProfiles/main/Profiles/"..saveplaceid.."/"..v2.OnlineProfileName
...
local SWSettings = GUI.CreateDivider2("SW Settings")
local StreamerModeToggle = {Enabled = false}
SilentwareFunctions.Controllers:register("UpdateUI", {UIUpdate = Instance.new("BindableEvent")})
SilentwareFunctions.Controllers:get("UpdateUI").UIUpdate.Event:Connect(function(h,s,v)
	GuiLibrary.UpdateUI(h,s,v)
end)
StreamerModeToggle = SWSettings.CreateToggle({
	Name = "StreamerMode",
...
GUIColorSlider = GUI.CreateColorSlider("GUI Theme", function(h, s, v)
	...
	SilentwareFunctions.Controllers:get("UpdateUI").UIUpdate:Fire(h,s,v)
end)
...
local Rewrite_Windows_Corresponder = {["Funny"] = "Blatant",["Hot"] = "Blatant",["Exploits"] = "Blatant",["Customisation"] = "Utility",["TP"] = "World",["Silentware"] = "Utility"}
...
SWFunctions.GlobaliseObject('makegradient', makegradient)
SWFunctions.GlobaliseObject('delgradient', delgradient)
...
local function hookObject(obj, property, target)
	...
	table.insert(SilentwareFunctions.Connections, con)
...
if shared.BACKUPTELEPORTMODE then
	teleportConnection = playersService.LocalPlayer.OnTeleport:Connect(function(State)
		if (not teleportedServers) and (not shared.VapeIndependent) then
			...
			local teleportScript = [[
				...
				if shared.VapeDeveloper or shared.VoidDev then
					if isfile('vape/NewMainScript.lua') then
						...
						loadstring(game:HttpGet("https://raw.githubusercontent.com/VapeSilentware/vapesilentware/main/NewMainScript.lua", true))()
					end
				else
					loadstring(game:HttpGet("https://raw.githubusercontent.com/VapeSilentware/vapesilentware/main/NewMainScript.lua", true))()
				end
			...
			if shared.NoSilentwareModules then
				teleportScript = 'shared.NoSilentwareModules = true\n'..teleportScript
			end
	...
	if (not shared.VapeIndependent) and (not shared.NoAutoExecute) then
		task.spawn(function()
			pcall(function()
				local teleportScript = [[
					repeat task.wait() until game:IsLoaded()
					if (not shared.SWEXECUTED) then
						shared.VapeSwitchServers = true
						shared.SWEXECUTED = true
						if shared.VapeDeveloper or shared.VoidDev then
							if isfile('vape/NewMainScript.lua') then
								loadstring(readfile("vape/NewMainScript.lua"))()
							else
				...
				local settings = {
					...
					{variable = "NoSilentwareModules", value = true},
...
local function InfoNotification(title, text, delay)
	local suc, res = pcall(function()
		local frame = GuiLibrary.CreateNotification(title or "Silentware", text or "Successfully called function", delay or 7, "assets/InfoNotification.png")
		return frame
	end)
    warn(title..": "..text)
	return (suc and res)
...
local function loadVape()
	InfoNotification("Silentware", "Loading...this might take 5-10 seconds", 5)
	--game:GetService("Players").LocalPlayer.GameplayPaused = true
	if not shared.VapeIndependent then
		pload("Universal.lua", true)
		pload("SWUniversal.lua", true)
		local fileName1 = "CustomModules/"..game.PlaceId..".lua"
		local fileName2 = "CustomModules/SW"..game.PlaceId..".lua"
		--local fileName3
		...
		if isGame then
			...
			fileName2 = "CustomModules/SW6872274481.lua"
			--if (not shared.CheatEngineMode) then fileName3 = "CustomModules/S6872274481.lua" end
		end
		if isLobby then
			fileName1 = "CustomModules/"..CE.."6872265039.lua"
			fileName2 = "CustomModules/SW6872265039.lua"
		end
		--if CE == "CE" then InfoNotification("Silentware", "Backup mode activated!", 3) end 
	...
	if shared.CheatEngineMode then
		if shared.CheatEngineMode and (not shared.VapeSwitchServers) then InfoNotification("Silentware", "Loaded in Cheat Engine Mode! Some functions might be missing.", 1.5) end
	else
		InfoNotification("Silentware", "Successfully loaded Silentware :D", 1.5)
	end
