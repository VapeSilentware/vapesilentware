...
SWFunctions.GlobaliseObject = function(name, obj)
    getgenv()[tostring(name)] = obj
    shared[tostring(name)] = obj
    table.insert(SWFunctions.GlobalisedObjects, {Name = name, Object = obj})
end

SWFunctions.Controllers = {}
function SWFunctions.Controllers:get(name)
    return SWFunctions.Controllers[tostring(name).."Controller"]
end
function SWFunctions.Controllers:register(name, tbl)
    SWFunctions.Controllers[tostring(name).."Controller"] = tbl
    SWFunctions.GlobaliseObject(tostring(name).."Controller", tbl)
end

SWFunctions.SelfDestructEvent = Instance.new("BindableEvent")
table.insert(SWFunctions.Connections, SWFunctions.SelfDestructEvent.Event:Connect(function()
    for i,v in pairs(SWFunctions.GlobalisedObjects) do getgenv()[tostring(v.Name)] = nil; shared[tostring(v.Name)] = nil end
    for i,v in pairs(SWFunctions.Connections) do if v.Disconnect then pcall(function() v:Disconnect() end) end end
    table.clear(SWFunctions)
end))

function SWFunctions.Connections:register(con)
    if (not con) then warn(debug.traceback("[SWFunctions.Connections:register]: con is nil!")) return end
    table.insert(SWFunctions.Connections, con)
end

local Base64 = {}

local b = 'ABCDEFGHIJKLMNOPQRSTU'..'V'..'WXYZabcdefghijklmnopqrstu'..'v'..'wxyz0123456789+/'
...
SWFunctions.Base64 = Base64

SWFunctions.handlepcall = function(suc, err)
    if suc == nil then return "nil suc" end
    if (not suc) then
        warn(debug.traceback("[SWFunctions.handlepcall]: Error in pcall! Error: \n"..tostring(err)))
    end
end
    ...
    ["Bedwars"] = {
        vapeGithubRequest = function(scripturl)
            if not isfile("vape/"..scripturl) then
                local suc, res = pcall(function() return game:HttpGet("https://raw.githubusercontent.com/VapeSilentware/vapesilentware/main/"..scripturl, true) end)
                assert(suc, res)
...
SWFunctions.LoadFunctions = function(game)
    if GamesFunctions[game] then
        for i,v in pairs(GamesFunctions[game]) do SWFunctions.GlobaliseObject(i,v) end
    end
end

SWFunctions.LoadServices = function()
    local services = {
    ...
    for i,v in pairs(services) do SWFunctions.GlobaliseObject(i,v) end
end

SWFunctions.EditWL = function(argTable)
    local NewTag_text
    local NewTag_color
    ...
    if type(argTable) == "table" and argTable["api_key"] then
        ...
        if NewTag_text or NewTag_color or Roblox_Username then
            ...
            local url = "https://whitelist.vapesilentware.xyz/edit_wl"
            local a = request({
                Url = url,
                Method = 'POST',
                Headers = headers,
...
SWFunctions.fetchCheatEngineSupportFile = function(fileName)
    local url = "https://raw.githubusercontent.com/VapeSilentware/SWCE/main/CheatEngine/"..tostring(fileName)
    local suc, res = pcall(function()
...
getgenv().SilentwareFunctions = SWFunctions
